/**
 * Portfolio component
 *
 * Highlights some of  your creations. These can be designs, websites,
 * open source contributions, articles you've written and more.
 *
 * This is a great area for you to to continually add to and refine
 * as you continue to learn and create.
 */

import React from "react";

/**
 * Desk image
 *
 * Below is a sample desk image. Feel free to update this to an image of your choice,
 * updating below imageAltText to string that represents what you see in that image.
 *
 * Need an image? Check out https://unsplash.com to download a photo you
 * freely use on your site.
 */
import image from "../images/design-desk.jpeg";

const imageAltText = "desktop with books and laptop";

/**
 * Project list
 *
 * An array of objects that will be used to display for your project
 * links section. Below is a sample, update to reflect links you'd like to highlight.
 */
const projectList = [
  {
    title: "TMDB-DataAnalysis",
    description:
      "This project aim to analyse data to a data set of films showing a visualization to the insights",
    url: "https://github.com/Amr-Ahmedd/TMDB-Data-Analysis",
  },
  {
    title: "Bike-Share-Data-Analysis",
    description:
      "This project involves analyzing a dataset of over 900,000 rows related to bike usage in the USA, extracting meaningful insights to inform strategic decisions for the bike share application.",
    url: "https://github.com/Amr-Ahmedd/Bike-Share-Data-Analysis",
  },
  {
    title: "My Resume",
    description:
      "You can view and download my CV from here",
    url: "https://drive.google.com/file/d/1VEvzu48ue-wYkXtS9NMY_UdAFYwoFlfR/view?usp=sharing",
  },
  {
    title: "Microsoft Build After Party Egypt  Whats new in GitHub Copilot",
    description:
      "I delivered an engaging session at the Microsoft Build After Party in Egypt, where I covered the latest features and advancements in GitHub Copilot",
    url: "https://stdntpartners-my.sharepoint.com/:v:/g/personal/amr_ahmed_studentambassadors_com/EYigtoLActBMpc4yruHKwCIB3j0mVRTO6TDKeIZ30aHKWg?e=j8H4ef&nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJTdHJlYW1XZWJBcHAiLCJyZWZlcnJhbFZpZXciOiJTaGFyZURpYWxvZy1MaW5rIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXcifX0%3D",
  },
];

const Portfolio = () => {
  return (
    <section className="padding" id="portfolio">
      <h2 style={{ textAlign: "center" }}>Portfolio</h2>
      <div style={{ display: "flex", flexDirection: "row", paddingTop: "3rem" }}>
        <div style={{ maxWidth: "40%", alignSelf: "center" }}>
          <img
            src={image}
            style={{ height: "90%", width: "100%", objectFit: "cover" }}
            alt={imageAltText}
          />
        </div>
        <div className="container">
          {projectList.map((project) => (
            <div className="box" key={project.title}>
              <a href={project.url} target="_blank" rel="noopener noreferrer">
                <h3 style={{ flexBasis: "40px" }}>{project.title}</h3>
              </a>
              <p className="small">{project.description}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default Portfolio;
